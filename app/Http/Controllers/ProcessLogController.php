        ...
        try {
            ...
            if ($step->order_no == 13) {
                ...
                if (config('services.fileserver.enable.post')) {
                    $chequeFile = $request->file('cheque-image-proof');
                    if (! $chequeFile) {
                        return redirect()->back()->with('error', 'Check proof image is required');
                    }

                    $extension = $chequeFile->getClientOriginalExtension();
                    $this->imageServices->post($application->originalClaimant()->client->tracking_no.'-cheque-proof', $chequeFile);
                    $application->update(['status' => 'released']);
                } else {
