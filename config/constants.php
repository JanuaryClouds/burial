<?php

/*
|--------------------------------------------------------------------------
| Constant Variables
|--------------------------------------------------------------------------
|
| These are mostly reusable strings used to ensure consistency between
| Livewire component names, actions, events, route names, and
| notification keys. This helps prevent typos and makes the codebase
| easier to maintain.
|
*/

return [
    'errors' => [
        'validation' => 'Please make sure all required fields are not empty. Follow the warning below the field.',
        'unauthorized' => 'You do not have the permission for this.',
        'unknown' => 'Something went wrong. Please try again later. If the problem persists, please contact the administrator.',
    ],
    'title' => [
        'main' => 'Funeral Assistance System',
        'sub' => 'CSWDO Taguig City',
    ],
];
