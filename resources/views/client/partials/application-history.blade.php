        ...
        @foreach ($records as $client)
            <div class="card bg-body-secondary">
                <div class="card-header collapsible cursor-pointer rotate d-flex align-items-center justify-content-between"
                    data-bs-toggle="collapse" data-bs-target="#client_info_card_body-{{ $client->tracking_no }}">
                    <h4 class="card-title">
                        {{ $client->tracking_no }} - Submitted on {{ $client->created_at->format('m/d/Y') }}
                    </h4>
                    <i class="ki-duotone ki-down fs-1"></i>
                </div>
                <div class="card-body collapse" id="client_info_card_body-{{ $client->tracking_no }}">
