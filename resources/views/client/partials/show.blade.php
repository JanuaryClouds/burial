...
@php
	if (isset($client)) {
	    ...
	    if ($socialInfo && Route::is('general.intake.form', 'application.create')) {
	...
	<div class="row">
		...
		<div class="col-4 col-md-4 col-lg-1">
			<x-form-input name="age"
				...
				required="true"
				type="number"
				value="{{ isset($client) ? $client->age() : session('citizen')['age'] ?? null }}"
				:readonly="$readonly" />
		</div>
		<div class="col-4 col-md-4 col-lg-1">
		...
		<div class="col-6 col-md-6 col-lg-2">
			...
			<x-form-select name="district_id"
				label="5.4. District"
				required="true"
				:options="$districts ?? []"
				:selected="$client->district_id ?? ''"
				:disabled="true"
				id="district_id_display" />
		</div>
