<div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div x-show="showDeleteModal" x-cloak x-transition:enter="transition ease-out duration-500"
        ...
        <div class="mt-5 mb-3 text-gray-700 text-200 text-lg">
            ...
            <p class="mt-2"><span class="font-semibold">Tracking no:</span> {{ $record->tracking_no }}</p>
            <p><span class="font-semibold">Name:</span> {{ $record->first_name }}
                {{ $record->middle_name }} {{ $record->last_name }}</p>
            <p><span class="font-semibold">Address:</span> {{ $record->house_no }} {{ $record->street }}
                {{ $record->barangay->name }} {{ $record->city }}</p>
        </div>
