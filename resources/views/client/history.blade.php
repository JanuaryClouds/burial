        ...
        <div class="card">
            <div class="card-header collapsible cursor-pointer rotate d-flex align-items-center justify-content-between"
                ...
                <span class="d-flex gap-2">
                    <p class="text-muted mb-0">(Latest record from {{ $client->created_at->format('m/d/Y') }})</p>
                ...
                <ul class="nav nav-tabs nav-line-tabs nav-stretch fs-4 border-bottom-0">
                    ...
                    <li class="nav-item">
                        <a href="#burials_tab" class="nav-link text-gray-700 pb-6" data-bs-toggle="tab">
                            Burial Assistances
                            <span
                                class="badge rounded-pill text-bg-light ms-2">{{ $records->sum(fn($c) => $c->claimant?->where('client_id', $c->id)->count()) ?? 0 }}</span>
                        </a>
                    </li>
                    <li class="nav-item">
