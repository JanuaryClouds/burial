...
@section('content')
    ...
    <div class="container-fluid flex-column align-items-center gap-4">
        <div class="row col">
            <form action="#" method="post" enctype="multipart/form-data" id="burialServiceForm"
                class="row flex-column w-75 mx-auto gap-4">
                @csrf
                @method('post')
                    ...
                    <div class="row row-gap-2">
                        ...
                        <div class="d-flex justify-content-between align-items-start gap-2">
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <input type="text" readonly value="{{ $service->deceased_lastname }}"
                                    name="deceased_lastname" id="deceased_lastname" class="form-control" disabled>
                            ...
                            <span class="d-flex flex-column w-75 justify-content-between">
                                <input type="text" disabled value="{{ $service->deceased_firstname }}"
                                    name="deceased_firstname" id="deceased_firstname" class="form-control">
                                <label for="deceased_firstname" class="text-sm form-label text-center">First Name
                                    M.I.*</label>
                            </span>
                        </div>
                        ...
                        <div class="d-flex justify-content-start align-items-center w-100 gap-1">
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <input type="text" disabled value="{{ $service->representative }}" name="representative"
                                    id="representative" class="form-control">
                                <label for="representative" class="text-sm form-label text-center">Full Name*</label>
                            </span>
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <input type="text" disabled value="{{ $service->representative_phone }}"
                                    name="representative_phone" id="representative_phone" class="form-control disabled">
                                <label for="representative_phone" class="text-sm form-label text-center">Phone (Mobile /
                                    Landline)*</label>
                            </span>
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <input type="text" disabled value="{{ $service?->representative_email ?? 'N/A' }}"
                                    name="representative_email" id="representative_email" class="form-control disabled">
                                <label for="representative_email" class="text-sm form-label text-center">Email*</label>
                            </span>
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <select required name="representative_relationship" id="representative_relationship"
                                    class="form-select" {{ $service ? 'readonly disabled' : '' }}>
                                    @if ($service)
                                        <option value="{{ $service->representative_relationship }}">
                                            {{ $relationships->firstWhere('id', $service->representative_relationship)->name ?? 'Unknown' }}
                                        </option>
                                    @else
                                ...
                                <label for="representative_relationship" class="text-sm form-label text-center">Relationship
                        ...
                        <div class="d-flex flex-column gap-2">
                            ...
                            <div class="d-flex justify-content-between align-items-center w-100 gap-1">
                                <span class="d-flex flex-column w-75 justify-between">
                                    <input type="text" disabled value="{{ $service->burial_address }}"
                                        name="burial_address" id="burial_address" class="form-control">
                                    <label for="burial_address" class="text-sm form-label text-center">Building Number,
                                ...
                                <span class="d-flex flex-column w-25 justify-content-between">
                                    <select name="barangay_id" id="barangay_id" class="form-select"
                                        ...
                                        @if ($service)
                                            <option value="{{ $service->barangay_id }}">
                                                {{ $barangays->firstWhere('id', $service->barangay_id)->name ?? 'Unknown' }}
                                            </option>
                                        @else
                            ...
                            <div class="d-flex align-items-center w-100 gap-4">
                                ...
                                <span class="d-flex justify-content-between align-items-center gap-2">
                                    ...
                                    <input type="date" required name="start_of_burial" id="start_of_burial"
                                        class="form-control" {{ $service ? 'hidden' : '' }}>
                                    <p class="text-sm text-gray-700 pt-3 fw-semibold {{ $service ? '' : 'hidden' }}">
                                        {{ $service ? Str::limit($service->start_of_burial, 10) : '' }}</p>
                                </span>
                                <span class="d-flex justify-content-between align-items-center gap-2">
                                    <label for="end_of_burial" class="form-label text-center">End*</label>
                                    <p class="text-sm text-gray-700 pt-3 fw-semibold {{ $service ? '' : 'hidden' }}">
                                        {{ $service ? Str::limit($service->end_of_burial, 10) : '' }}</p>
                                    <input type="date" required name="end_of_burial" id="end_of_burial"
                                        class="form-control" {{ $service ? 'hidden' : '' }}>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center w-full gap-2">
                            <span class="d-flex flex-column justify-content-between w-75">
                                <select name="burial_service_provider" disabled id="burial_service_provider"
                                    class="form-select">
                                    @if ($service)
                                        <option value="{{ $service->burial_service_provider }}"
                                ...
                                <label for="burial_service_provider" class="form-label text-center">Burial/Funeral
                                    Service*</label>
                            </span>
                            <span class="d-flex flex-column w-25 justify-content-between">
                                <span class="input-group">
                                    <span class="input-group-text">₱</span>
                                    <input type="text" name="collected_funds" id="collected_funds" disabled
                                        value="{{ $service ? $service->collected_funds : '' }}" class="form-control" />
                        ...
                        <div class="d-flex justify-content-between align-items-center w-full gap-2">
                            <span class="d-flex flex-column justify-content-between w-75">
                                <input type="text" name="burial_service_address" id="burial_service_address" disabled
                                    value="{{ $service ? $service?->provider?->address : '' }}, {{ $service ? $service?->provider?->barangay?->name : '' }}"
                                    class="form-control">
                                <label for="burial_service_address" class="form-label text-center mb-0">Address of Burial
                                    Service</label>
                            </span>
                            <span class="d-flex flex-column justify-content-between w-25">
                                <input type="text" value="{{ $service ? $service?->provider?->contact_details : '' }}"
                                    name="burial_service_contact" id="burial_service_contact" disabled
                                    class="form-control">
        ...
        <div class="row justify-content-center align-items-center g-2 mt-3">
            <div class="col d-flex justify-content-center">
                <a name="" id="" target="_blank" class="btn btn-primary"
                    href="{{ route('admin.burial.service.print', ['id' => $service->id]) }}" role="button">Export
                    PDF</a>
            </div>
        </div>
