...
@if (Request::routeIs('general.intake.form', 'application.create'))
...
<div class="row flex-column justify-content-center align-items-center g-2">
	@if (Route::is('general.intake.form', 'application.create'))
		@if (config('services.fileserver.enable.post'))
			@foreach ($documents as $document)
				...
				<div class="col mb-4 {{ $document['is_muslim'] ? 'muslim-requirements' : '' }}">
					<x-form-image-submission name="images[{{ $document['key'] }}]"
						label="{{ $displayName }}"
