...
<script nonce="{{ $nonce ?? '' }}">
    ...
    document.getElementById('preset-year-' + uid).addEventListener('click', function() {
        document.getElementById('start_date_' + uid).value = '{{ now()->startOfYear()->format('Y-m-d') }}';
        document.getElementById('end_date_' + uid).value = '{{ now()->endOfYear()->format('Y-m-d') }}';
        this.form.submit();
    });
    ...
    document.getElementById('preset-month-now-' + uid).addEventListener('click', function() {
        document.getElementById('start_date_' + uid).value = '{{ now()->startOfMonth()->format('Y-m-d') }}';
        document.getElementById('end_date_' + uid).value = '{{ now()->endOfMonth()->format('Y-m-d') }}';
        this.form.submit();
    });
</script>
